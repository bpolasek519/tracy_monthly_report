...
def create_wip_df(df: pd.DataFrame, title: str, for_fs: bool = True) -> pd.DataFrame:
    ...
    final_df = df_percent_filter[cols_to_keep]
...
def create_outstanding_df(df: pd.DataFrame, title: str) -> pd.DataFrame:
    # Filter so job type is only JOC or HB
    mask_job_type = df['Type:  JOC, CC, HB'].str.contains('JOC|HB', case=False, na=False)
    df_filtered_for_job_type = df[mask_job_type].copy()
    ...
    mask_billed_date_blank = df['Billed Date'].isna()
...
def create_paid_df(df: pd.DataFrame, title: str) -> pd.DataFrame:
    # Filter so job type is only JOC or HB
    mask_job_type = df['Type:  JOC, CC, HB'].str.contains('JOC|HB', case=False, na=False)
    df_filtered_for_job_type = df[mask_job_type].copy()

    # Filter for paid in the current month
    df['Paid/   Closed'] = pd.to_datetime(df['Paid/   Closed'])
    mask_paid_current_month = df['Paid/   Closed'].dt.month == 7
    df_filtered_for_paid_current_month = df_filtered_for_job_type[mask_paid_current_month]
